...
import (
	...
	winoptions "github.com/liugangnhm/wails/v2/pkg/options/windows"
)
...
type Window struct {
	...
	// 标识窗口是否处于最小化状�?用于解决最小化/恢复时的闪屏问题
	// This flag is used to prevent unnecessary redraws during minimize/restore transitions for frameless windows
	// 此标志用于防止无边框窗口在最小化/恢复过程中的不必要重�?	// Reference: https://github.com/wailsapp/wails/issues/3951
	isMinimizing bool
}
...
// 包装了chromium.SetPadding的过滤器,用于防止窗口最小化/恢复过程中的不必要重�?// This fixes window flickering when minimizing/restoring frameless windows
...
func (w *Window) SetPadding(padding edge.Rect) {
	...
	// 如果窗口正在最小化,跳过设置padding以防止闪�?	if w.isMinimizing {
		return
	}
